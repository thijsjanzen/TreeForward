...
public:
  ...
  void setup(long seedin , long J_M_in , double mu_in , double s_in)
  {
    ...
    // note there is also a system restore function lower down in this file
    total_clear();
    set_seed(seedin);
                 ...
                 double S)
{
  ...
  Rcout << "simulating M with fitness increase or decrease ... ";
  Rcout << JM << " , " << nu << " , " <<  S << " \n";

  test.sim_all(get_seed() , JM , nu , S , 60 * task_max);
